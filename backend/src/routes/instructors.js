...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await InstructorsService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.put('/:id', wrapAsync(async (req, res) => {
  await InstructorsService.update(req.body.data, req.body.id, req.currentUser);
  const payload = true;
  res.status(200).send(payload);
}));
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await InstructorsDBApi.findAll(
    req.query, { currentUser }
  );
  if (filetype && filetype === 'csv') {
    const fields = ['id','first_name','last_name','qualifications',
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await InstructorsDBApi.findBy(
    { id: req.params.id },
  );
