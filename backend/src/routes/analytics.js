...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await AnalyticsService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await AnalyticsDBApi.findAll(
    req.query, { currentUser }
  );
  if (filetype && filetype === 'csv') {
    const fields = ['id',
        'student_engagement','completion_rate','instructor_performance',
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await AnalyticsDBApi.findBy(
    { id: req.params.id },
  );
